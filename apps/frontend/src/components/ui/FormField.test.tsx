...
describe('FormField', () => {
  ...
  it('adds margin-top to content when label is present', () => {
    const { container } = render(
      <FormField label="With Label">
        <input type="text" />
      </FormField>,
    );
    const contentDiv = container.querySelector('label + div');
    expect(contentDiv).toHaveClass('mt-2');
  });

  it('does not add margin-top when label is absent', () => {
    const { container } = render(
      <FormField>
        <input type="text" />
      </FormField>,
    ...
    expect(contentDiv).not.toHaveClass('mt-2');
  });
});
