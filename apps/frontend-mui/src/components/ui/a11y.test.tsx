...
describe('UI Components A11y Tests', () => {
  ...
  describe('FormField', () => {
    ...
    // Note: MUI FormControl handles required differently than native HTML
    // The required prop is passed to children via render prop
    it('should render required field correctly', async () => {
      const { container } = render(
  ...
  describe('Select', () => {
    it('should have no accessibility violations', async () => {
      ...
      const { container } = render(
        // MUI TextField select uses label prop for accessibility
        <Select id="test-select" label="Choose an option" options={options} />,
      );
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
